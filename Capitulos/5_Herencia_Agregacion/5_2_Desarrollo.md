...
Los CD y los DVD son discos. Hasta lucen similares. Aunque no puedo ver la película contenida en un DVD en el reproductor de discos compactos y no puedo escuchar la música contenida en un CD en el reproductor de DVD, guardo todos mis discos en una discoteca.
...
```
> [Ver en repositorio »](https://github.com/ucudal/PII_Herencia/blob/master/src/Library/Disc.cs)

> En este ejemplo en C# aparece por primera vez la palabra clave `abstract`. Cuando aparece junto al nombre de la clase significa que la clase es abstracta, es decir, que no puede tener instancias; dicho de otra forma, una clase abstracta se define con el único propósito de ser heredada. Cuando la palabra clave `abstract` aparece en la declaración de un método, significa que el método es abstracto, es decir, que no tiene implementación; dicho de otra forma, la implementación se deja para los sucesores. Una clase que tiene un método abstracto es necesariamente una clase abstracta, pero una clase abstracta no tiene porqué tener sus métodos abstractos.
